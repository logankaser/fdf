...
/*   Updated: 2018/12/17 20:21:20 by lkaser           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vec3	*parse_line(t_fdf *map, char *l, int y)
{
	int		i;
	int		x;
	int		len;
	...
	while (l[i] && ++x < len)
	{
		...
		while (l[i] && l[i] == ' ')
			++i;
		while (l[i] && (ft_isdigit(l[i + 1]) || l[i + 1] == '-'))
			++i;
		i = l[i] ? i + 1 : 0;
	}
...
static t_fdf	*read_map(char *mapfile)
{
	t_fdf		*map;
	int			fd;
	char		*line;
